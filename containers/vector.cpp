...
int main() {
    ...
    // 1. using [] to add and modify element
    v1[0] = 11;
    v1[1] += 2; // modify directly
    v1[2]++;

    // 2. using insert to add element but does not overwrite
    auto it = v1.insert(v1.end(), 3);
    print(v1);

    // 3. using emplace_back or push_back to insert
    v1.emplace_back(60); // more performant construct the object directly in container
    ...
    // 4. some different ways to copy vectors
    auto v2{v1};
